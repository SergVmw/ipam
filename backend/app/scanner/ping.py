...
async def sweep(cidr: str, method: str = "auto", timeout_ms: int = 500, rate: int = 500, ports: list[int] | None = None) -> dict[str, dict]:
    ...
    if m == "nmap":
        ...
        return await nmap_sweep(cidr, rate)
    alive = await tcp_probe(cidr, timeout_ms, ports or [22, 80, 443, 3389])
    return {ip: {"mac": None, "vendor": None} for ip in alive}


async def nmap_sweep(cidr: str, rate: int) -> dict[str, dict]:
